...
if(strlen($company)>0){
    ...
    $id = $mysqli->insert_id;
    $res = $mysqli->query("SELECT * FROM startupfair WHERE id=".$id);
    $row = $res->fetch_assoc();
?>
<div class="alert alert-block">
  <h4 class="alert-heading">Create new startup</h4>
  <p>Here's the link: http://www.acm.uiuc.edu/conference/2012/startupfair-registration.php?company=<?= $row['company']; ?>&key=<?= $row['hash']; ?>
</div>
